...
import { Attribute, Class, Compare, PredicateExists, SelectorFilter, SelectorFind, SugarElement, SugarNode, Traverse } from '@ephox/sugar';
...
const findMarkers = (editor: Editor, uid: string): Array<SugarElement<Element>> => {
  ...
  return Arr.filter(descendants, (descendant) => !isOffscreenSelectionElement(descendant, body));
};
